...
class Detection:
    ...
    def in_conv(self, cmd, person_id, x1, y1, x2, y2):
        ...
        self.is_start = True
        if self.cmd[person_id] != "end":
            if person_id not in self.tracer_dict.keys():
                self.tracer_dict[person_id] = np.array([[y1, y2]])
                self.null_count[person_id] = 0
            else:
                np_tmp = self.tracer_dict[person_id]
                tmp = np.array([[y1, y2]])
                self.tracer_dict[person_id] = np.append(np_tmp, tmp, axis=0)
                self.null_count[person_id] = 0

            for person in self.null_count.keys():
                if person != person_id:
                    self.null_count[person] += 1
        ...
        for _, (person, comand) in enumerate(self.cmd.items()):
            if comand == "end":
                ...
                kill.append(person)
        for person in kill:
            del self.null_count[person]
            del self.tracer_dict[person]
            del self.cmd[person]
...
@torch.no_grad()
def run(
    source="B1_Cam1_1.mp4",  # "rtsp://aifoundry:Coieeb1(@140.134.208.212:554/chID=0&streamType=main"
            ...
            if det is not None and len(det):
                ...
                if len(outputs[i]) > 0:
                    for j, (output, conf) in enumerate(zip(outputs[i], confs)):
                        bboxes = output[0:4]
                        act = detect.in_conv(
                            "add", output[4], bboxes[0], bboxes[1], bboxes[2], bboxes[3]
                        )
                        if act == "end":
                            print("end")
                            f.write("end\n")
