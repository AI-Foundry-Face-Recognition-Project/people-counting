...
class Alg(AlgBase):
    ...
    def create_model(self, model_name='yolov3_tiny', dev='cpu'):
        ...
        try:
            ...
            model_cfg=os.path.join('./model_zoo/YoloV3/config/yolov3-tiny.cfg')
            model_weight=os.path.join('./model_zoo/YoloV3/yolov3-tiny.weights')
            self.model = load_model(model_cfg, model_weight)
            if self.device == "cuda":
